...
mkdir -p "$TOOLS_DIR/cache" "$TOOLS_DIR/src" "$TOOLS_DIR/build" "$WHISPER_BIN_DIR"
...
# Build whisper.cpp tools locally if missing.
need_build=0
if [[ ! -x "$WHISPER_BIN_DIR/whisper-cli" ]]; then
  need_build=1
fi
if [[ ! -x "$WHISPER_BIN_DIR/whisper-server" ]]; then
  need_build=1
fi
